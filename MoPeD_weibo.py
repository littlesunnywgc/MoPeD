...
parser.add_argument('--fc4_in_features', type=int, default=2450, help='Number of input features for fc4')
parser.add_argument('--fc3_in_features', type=int, default=1550, help='Number of output features for fc3')
parser.add_argument('--fc2_in_features', type=int, default=700, help='Number of input features for fc2')
...
class NeuralNetwork(nn.Module):
    ...
    def fit(self, X_train_tid, X_train, y_train,X_dev_tid, X_dev, y_dev,image_features1, text_features1,image_features2, text_features2):

        if torch.cuda.is_available():
            self.cuda()
        batch_size = self.config['batch_size']
        self.optimizer = torch.optim.Adam(self.parameters(), lr=1e-3, weight_decay=0)#这里这里！！！
...
class resnet50():
    def __init__(self):
        ...
        self.path = '../dataset/weibo/imagesall/'
    ...
    def img_trans(self):
        transform = transforms.Compose([
            ...
            transforms.Lambda(lambda x: x.convert("RGB") if x.mode != "RGB" else x),  # 如果是灰度图像，转换为RGB模式
            transforms.ToTensor(),
    ...
    def forward(self,X_tid):
        ...
        for newid in X_tid.cpu().numpy():
            img = self.X_img[newid]
            jpg_path = self.path + img + '.jpg'
            gif_path = self.path + img + '.gif'
...
def load_dataset():
    ...
    X_train_tid, X_train, y_train,word_embeddings = pickle.load(open(pre + "/train.pkl", 'rb'))
    X_dev_tid, X_dev, y_dev = pickle.load(open(pre + "/dev.pkl", 'rb'))
    X_test_tid, X_test, y_test = pickle.load(open(pre + "/test.pkl", 'rb'))
    config['embedding_weights'] = word_embeddings

    train_image ,test_image =pickle.load(open(pre + "/X_img.pkl", 'rb'))
    X_img = train_image + test_image
    config['X_img'] = X_img
...
def train_and_test(model):
    model_suffix = model.__name__.lower().strip("text")+ '_' +"clip_kl"
    res_dir = 'exp_result'
    ...
    image_features1, text_features1, y_train, \
    image_features2, text_features2, y_dev, \
    image_features3, text_features3, y_test = load_dataset()
